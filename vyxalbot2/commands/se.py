...
class SECommands(CommonCommands):
    ...
    async def helpCommand(self, event: EventInfo, command: str = ""):
        ...
        if command:
            ...
            yield self.common.messages["help"] + ", ".join(
                sorted(
                    set(
                        map(
                            lambda i: i.split(" ")[0],
                            event.service.commands.commands.keys(),
                        )
                    )
                )
            )

    async def getPermissionsTarget(
        self, event: EventInfo, name: str
    ) -> Union[User, str]:
        if name == "me":
    ...
    async def permissionsModify(
        self, event: EventInfo, name: str, group: str, grant: bool
    ):
        if isinstance(target := (await self.getPermissionsTarget(event, name)), str):
            yield target
            return
    ...
    async def pingCommand(self, event: EventInfo, group: str, message: str):
        ...
        pings = " ".join(
            [
                ...
                if target.serviceIdent != event.userIdent
            ]
        )
        if not len(pings):
    ...
    async def issueOpenCommand(
        self, event: EventInfo, repo: str, title: str, body: str, tags: list[str] = []
    ):
    ...
    async def issueCloseCommand(
        self, event: EventInfo, repo: str, num: int, body: str = ""
    ):
    ...
    async def trashCommand(
        self, event: EventInfo, startRaw: str, endRaw: str, target: int = TRASH
    ):
        ...
        async with ClientSession() as session:
            ...
            if (await getRoomOfMessage(session, start)) != self.common.privateConfig[
                "chat"
            ]["room"]:
            ...
            identRange = [
                ...
                async for i in getMessageRange(
                    session, self.common.privateConfig["chat"]["room"], start, end
                )
            ]
